...
def test_create_user_new_email(
    ...
    with (
        ...
        created_user = r.json()
        user = repositories.get_user_by_email(session=db, email=username)
        assert user
...
def test_get_existing_user_as_superuser(
    ...
    assert 200 <= r.status_code < 300
    api_user = r.json()
    existing_user = repositories.get_user_by_email(session=db, email=username)
...
def test_create_user_existing_username(
    ...
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    repositories.create_user(session=db, user_create=user_in)
    data = {"email": username, "password": password}
    r = client.post(
        f"{settings.API_V1_STR}/users/",
        headers=superuser_token_headers,
...
def test_retrieve_users(
    ...
    repositories.create_user(session=db, user_create=user_in2)
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = repositories.create_user(session=db, user_create=user_in2)
