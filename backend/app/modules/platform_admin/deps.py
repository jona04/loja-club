...
def require_platform_permission(permission: str) -> Callable[..., User]:
    ...
    The platform parallel of ``require_permission``, without a store scope: it
    resolves the user's global roles from ``platform_admin_roles`` and checks the
    in-code role->permission map. Authorization is enforced in the backend
