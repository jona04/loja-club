...
- **INV-I1 — Três identidades separadas:** admin de plataforma e lojista/equipe em `account_users`; cliente final em `customer_profiles`. → Fase 0 (accounts), Fase 6 (customers).
- **INV-I2 — Auth do cliente é um sistema separado** do `account_users` (token por loja; código/senha/Google). Projetar `accounts` sem acoplar a auth do cliente. → desenho em Fase 0 (`P0-MOD-04`), implementação Fase 8.
- **INV-I3 — Dedup do customer por loja** = e-mail normalizado e telefone E.164, match priorizando e-mail; primeiro-nome-vence. → Fase 6.
- **INV-I4 — Compra sem login** via `guest_session_id` (cookie HTTP-only). → Fase 6.
...
- **INV-S1 — Permissão por loja no backend** (membership + papel; + plano na Fase 8). → Fase 1, gating de plano Fase 8.
- **INV-S2 — Arte do cliente é privada** (URLs assinadas, separada por `store_id`). → Fase 2/6.
- **INV-S3 — Segredos fora do código** (env seguro/SSM). → todas; reforço Fase 9.
- **INV-S4 — LGPD/consentimento + minimização**; `customer_consents`. → Fase 6.
- **INV-S5 — Rate limit** em login/recuperação/checkout/códigos/APIs públicas. → baseline Fase 6, completo Fase 9.

## 9. Frontends

- **INV-FE1 — Três projetos separados:** `frontend-dashboard` (`app.`), `frontend-admin` (`admin.`), `frontend-storefront` (Next.js, `*.`). → Fases 1/3/8.
...
| DEC-2 | Arredondamento monetário | `ROUND_HALF_UP`; rateio de split por maior-resto | recomendado, confirmar antes da Fase 8 |
...
| Auth do cliente separada (desenho) | Fase 0 — `P0-MOD-04` → Fase 8 |
