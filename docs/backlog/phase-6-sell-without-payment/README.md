...
- [x] **Tela de Frete no painel** ✅ — `frontend-dashboard/src/routes/_layout/shipping.tsx` (lista + criar/editar/ativar-desativar/remover métodos, gated `shipping.*`) + menu "Frete" (`shipping.view`). Fecha a lacuna de UI do `P6-SHIP-01` (era backend-only) — desbloqueia configurar entrega sem Swagger. Origem: `P6-SHIP-01`.
