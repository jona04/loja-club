...
## Follow-ups
- [x] **Tela de Frete no painel** ✅ — `routes/_layout/shipping.tsx` (lista/criar/editar/ativar-desativar/remover, gated `shipping.*`) + menu "Frete"; usa o `ShippingService` já gerado (sem mudança no backend). Fecha a lacuna de UI desta task (era backend-only) — o lojista configura entrega sem Swagger. Origem: `P6-SHIP-01`.
