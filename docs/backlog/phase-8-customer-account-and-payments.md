...
Docs de referência: [12](../concepts/12_aws_infrastructure_and_deployment.md), [23](../concepts/23_customer_identity_and_guest_checkout.md), [11](../concepts/11_checkout_payments_and_split.md), [02](../concepts/02_business_model_and_rules.md), [08](../concepts/08_modules_and_permissions.md), [07](../concepts/07_database_strategy.md), [14](../concepts/14_security_strategy.md), [18](../concepts/18_open_decisions.md), [16](../concepts/16_testing_strategy.md).
...
- **Cobrança da mensalidade:** desde já, manual ou via gateway recorrente (doc [18](../concepts/18_open_decisions.md)).

---

## Etapa 1 — Deploy do ambiente dev na AWS (EC2)

> Subir **antes** dos pagamentos: o gateway envia webhooks para uma URL pública. Ambiente **dev online** (não é produção; produção ECS é pós-V1). Doc [12](../concepts/12_aws_infrastructure_and_deployment.md).
...
- [ ] **Route 53**: `*.loja.club`, `api.`, `app.` (e `admin.` quando a Fase 9 entrar). Doc [06](../concepts/06_multitenancy_and_domains.md)/[12](../concepts/12_aws_infrastructure_and_deployment.md).
- [ ] **SSL** via Traefik/Let's Encrypt (ACM fica para produção). Doc [12](../concepts/12_aws_infrastructure_and_deployment.md).
...
- [ ] Health checks acessíveis (`/health`, `/health/db`, `/health/redis`). Doc [15](../concepts/15_observability_and_operations.md).

### DoD da etapa
- [ ] Loja pública, painel e API acessíveis por domínio público com HTTPS; webhooks alcançáveis.

**Reconciliação:** ambiente dev online usa **EC2 + Docker Compose + Traefik** (doc [12](../concepts/12_aws_infrastructure_and_deployment.md)). ECS/Fargate é **pós-V1**.
...
- [ ] `payment_transactions`: `store_id`, `order_id`, `gateway_transaction_id`, `status` (`created|pending|authorized|paid|refused|canceled|refunded|chargeback`), `amount`, `method`, `metadata`. Índice `store_id+gateway_transaction_id`. Doc [07](../concepts/07_database_strategy.md).
...
- [ ] **Webhook:** validar assinatura/origem, **idempotência** (`gateway_event_id`), pertencimento à loja, status válido → atualizar transação + pedido. **Requer o sistema no ar (Etapa 1).** Doc [11](../concepts/11_checkout_payments_and_split.md)/[14](../concepts/14_security_strategy.md).
...
- [ ] Conectar conta; status da conta/métodos; transações; problemas; chargebacks; info de repasse (somente exibição — Loja Club não retém dinheiro). Doc [09](../concepts/09_merchant_dashboard.md)/[11](../concepts/11_checkout_payments_and_split.md).
...
**Reconciliação:** **nunca** armazenar cartão; dados sensíveis ficam no gateway (doc [14](../concepts/14_security_strategy.md)). A lib do gateway não existe no template.
...
- [ ] **Comissão por plano** alimenta o split do `payments` (Etapa 3). Lojista não configura comissão. Doc [11](../concepts/11_checkout_payments_and_split.md).
