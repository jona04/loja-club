...
- [ ] **e2e polui o db de dev (corrigir):** ambos `playwright`/`playwright-admin` batem no `backend` → db `app`; o **signup** dos testes **commita** usuários → acumulam (o pytest **não** polui — rollback). **Fix certo:** e2e contra **db descartável** (`db-test` + `backend-e2e` com `POSTGRES_DB` próprio), não o db de dev — **não** mascarar nos sample-tests (revertidos p/ `count==1`). Origem: e2e da Fase 4.
