...
## Contexto
O admin gerencia as **definições** de planos — a tabela **`billing_plans`** do módulo `billing` (doc [07](../../concepts/07_database_strategy.md)). O **billing real** (assinatura, cobrança, comissão, enforcement) é a **[Fase 8](../phase-8-customer-account-and-payments.md)** — aqui só as **definições** (seed/stub): existem e são editáveis; nada é cobrado/forçado e a loja ainda **não** assina.

## Docs de referência
- [02 — Business Model and Rules](../../concepts/02_business_model_and_rules.md) (planos Free/Pro)
- [07 — Database Strategy](../../concepts/07_database_strategy.md) (`billing_plans`, `billing_*`)
- [08 — Modules and Permissions](../../concepts/08_modules_and_permissions.md) (`platform.plans.view|update`)
- [25 — Platform Admin](../../concepts/25_platform_admin.md)

## Escopo (o que ENTRA)
- **`billing_plans`** (definições: nome, mensalidade, comissão %, observação) — **seed** dos **4 planos** do doc [02](../../concepts/02_business_model_and_rules.md): Free R$0/5% · Starter R$49,90/3% · Pro R$99,90/1,5% · Business R$199,90/0,5%.
...
- **Enforcement por plano** (bloquear recurso) → **Fase 8** (o gancho já existe em `require_permission`, Fase 1; doc [02](../../concepts/02_business_model_and_rules.md) §"plano + permissão").
- Telas → `P4-ADMIN-02`.
...
- Usa a tabela **definida** `billing_plans` (doc [07](../../concepts/07_database_strategy.md)), não uma tabela nova. O **resto do `billing`** (assinatura/cobrança/comissão/enforcement) é a **Fase 8**; a Fase 4 introduz só as **definições** (consistente com doc [25](../../concepts/25_platform_admin.md): "consome billing quando existir; na V1 seed/stub").
