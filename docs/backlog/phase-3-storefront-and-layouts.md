...
- [ ] `resolve_store_by_host` (consome `tenancy`/`domains` + cache `domain:{host}`).
...
- [ ] `content_store_theme_settings`: `store_id` (único), `active_template_id`, `logo_url`, `banner_image_url`, `headline`, `description`, `featured_collection_id`, e campos preparados para o futuro (`primary_color`, `background_color`, `font_family`). Doc [10](../10_storefront_and_layouts.md).
