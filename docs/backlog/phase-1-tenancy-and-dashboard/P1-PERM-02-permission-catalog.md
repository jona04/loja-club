...
## Contexto
O painel é dividido em módulos com **permissões granulares**; as regras são **positivas** (o que não está no papel é negado). O doc [08](../../concepts/08_modules_and_permissions.md) traz o catálogo e o mapa; o doc [07](../../concepts/07_database_strategy.md) lista `store_permissions` como **tabela**. Esta task cria o catálogo e o mapa **no banco** (seedados), sem inventar permissões.

## Docs de referência
- [08 — Modules and Permissions](../../concepts/08_modules_and_permissions.md) (catálogo + mapa por papel — **espelhar exatamente**)
- [07 — Database Strategy](../../concepts/07_database_strategy.md) (`store_permissions` como tabela)
- [09 — Merchant Dashboard](../../concepts/09_merchant_dashboard.md) (módulo ↔ permissão base)

## Escopo (o que ENTRA)
- `store_permissions` (`__tablename__="store_permissions"`): tabela **seedada** com o **catálogo completo** do doc [08](../../concepts/08_modules_and_permissions.md) (`key`, `module`, `description`): `dashboard.*`, `catalog.*`, `customization.*`, `orders.*`, `customers.*`, `checkout.*`, `payments.*`, `layout.*`, `shipping.*`, `discounts.*`, `reports.*`, `domains.*`, `team.*`, `billing.*`, `settings.*`.
- `store_role_permissions` (`__tablename__="store_role_permissions"`): join **seedado** (`role` → `permission`) que materializa o **mapa positivo** do doc [08](../../concepts/08_modules_and_permissions.md) para `owner|admin|manager|support|catalog|marketing`.
- **Fonte de seed em código** (`app/modules/stores/permissions.py`): o catálogo e o mapa do doc [08](../../concepts/08_modules_and_permissions.md) ficam como **constantes canônicas** que o `init_db` usa para popular as tabelas (single source do seed, idempotente — ver Notas) + helper `role_permissions(role)`.
...
- Overrides por membro (`store_member_permissions`, opcional no doc [07](../../concepts/07_database_strategy.md)) → fora do MVP.
...
- `docs/concepts/07_database_strategy.md` (alterar) — adicionar `store_role_permissions` à lista de tabelas/índices (join não estava explícito).

## Passos
1. Transcrever o catálogo e o mapa do doc [08](../../concepts/08_modules_and_permissions.md) para `permissions.py` (sem adicionar/remover).
...
- **Cobrir:**
  - unit — **toda permissão do catálogo aparece em ≥1 papel**; `owner` contém todas; conjuntos de `admin/manager/support/catalog/marketing` batem com o doc [08](../../concepts/08_modules_and_permissions.md).
...
## Definition of Done
- [x] `store_permissions` (63) e `store_role_permissions` criadas via migration `8d0ba011eeec` e **seedadas** por `init_db` (idênticas ao doc [08](../../concepts/08_modules_and_permissions.md)).
- [x] Unit (catálogo/mapa, com counts por papel) + integração (integridade do seed) verdes *(96 testes; cobertura 91%)*.
- [x] Doc [07](../../concepts/07_database_strategy.md) atualizado com `store_role_permissions`; regra registrada: **permissão nova → atualizar catálogo + mapa + doc**.
...
- O **join `store_role_permissions`** não estava explícito no doc [07](../../concepts/07_database_strategy.md) (que listava só `store_roles` e `store_permissions`); adicionado por ser a forma normalizada do "papel recebe uma lista explícita de permissões" (doc [08](../../concepts/08_modules_and_permissions.md)).
