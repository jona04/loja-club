...
- Dependency `require_permission("catalog.product.update")` que valida, em ordem:
  ...
  4. **gancho de plano** (stub que sempre permite no MVP; gating real na Fase 6).
...
- Lógica de **plano** (plano permite o recurso) → Fase 6 (só o gancho aqui).
- Overrides de permissão por membro (`store_member_permissions`) → criar só se necessário, fora desta task.
- Permissões globais de plataforma (`platform.*`) → Fase 7.
...
2. Adicionar o gancho de plano (no-op no MVP, ponto de extensão p/ Fase 6).
...
- [x] Gancho de plano presente (`_plan_allows()`, no-op) para a Fase 6 plugar.
...
- "Plano + permissão" (doc [08](../../08_modules_and_permissions.md)): no MVP só a permissão é ativa; o plano entra na Fase 6.
