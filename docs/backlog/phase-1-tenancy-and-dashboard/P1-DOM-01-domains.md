...
- [13 — Performance, Cache and CDN](../../concepts/13_performance_cache_and_cdn.md) (chave `domain:{host}`)
- [18 — Open Decisions](../../concepts/18_open_decisions.md) (domínio próprio fora do MVP)

## Escopo (o que ENTRA)
- `domain_hosts` (`__tablename__="domain_hosts"`): `id`, `store_id`, `host` (**único**), `type` (`platform_subdomain|custom_domain`), `status` (`pending|active|failed|blocked`), `ssl_status` (`pending|issued|failed`), `verified_at`, timestamps, soft delete. Doc [06](../../concepts/06_multitenancy_and_domains.md).
...
- **Domínio próprio** (`custom_domain` + verificação DNS + SSL) → fora do MVP (doc [18](../../concepts/18_open_decisions.md)); deixar só o campo/placeholder.
...
- **`ssl_status=issued`** para `platform_subdomain` (coberto pelo wildcard `*.{DOMAIN}`); `verified_at` fica nulo. Doc [06](../../concepts/06_multitenancy_and_domains.md): V1 **sem domínio principal** — qualquer host ativo renderiza a mesma loja.
