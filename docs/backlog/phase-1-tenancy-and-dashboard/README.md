...
- `account_users.is_superuser` (template) ↔ admin de plataforma (doc [08](../../08_modules_and_permissions.md)): no MVP o superuser cobre o acesso interno; `platform_admin_roles`/`platform.*` entram na Fase 4. (Herdado de `P0-MOD-04`.)
...
- **Gating por plano** (doc [08](../../08_modules_and_permissions.md) "plano + permissão") entra na Fase 8; aqui deixamos só o **gancho** em `require_permission`.
...
- [ ] **Políticas básicas da loja** (devolução/troca/privacidade) — doc [09](../../09_merchant_dashboard.md) lista em Configurações, mas é escopo de checkout (`checkout.policies.*`). Origem: vistoria da Fase 1. *Quando:* Fase 6 (checkout).
