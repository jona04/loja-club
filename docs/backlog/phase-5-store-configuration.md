...
Docs de referência: [26](../concepts/26_template_system.md) (sistema de templates), [27](../concepts/27_template_authoring_guide.md) (autoria de template), [09](../concepts/09_merchant_dashboard.md) (painel), [10](../concepts/10_storefront_and_layouts.md) (storefront), [25](../concepts/25_platform_admin.md) (de onde vêm os templates/schema/thumbnail), [07](../concepts/07_database_strategy.md), [13](../concepts/13_performance_cache_and_cdn.md), [16](../concepts/16_testing_strategy.md).
...
## Etapa 1 — Groundwork: e2e descartável + auditoria do doc de banco
...
- [ ] **Auditoria do doc de banco** (`P5-DOC-01`): conferir que toda tabela/índice criado (Fases 0–4) está no doc [07](../concepts/07_database_strategy.md); atualizar onde faltar (a doc é a fonte de verdade).

---

## Etapa 2 — Settings schema: storage + API (backend)
...
### Modelo (com `store_id`) (doc [07](../concepts/07_database_strategy.md)/[26](../concepts/26_template_system.md))
...
### Serviço/rotas (doc [26](../concepts/26_template_system.md))
...
## Etapa 3 — Form genérico no painel

### `TemplateSettingsForm` (doc [09](../concepts/09_merchant_dashboard.md)/[26](../concepts/26_template_system.md))
...
- [ ] Upload de imagem (campos `image`) reusa `media`. **Fix:** o upload no contexto de **layout** deve ser gated em **`layout.assets.update`** (hoje o endpoint de `media` exige `catalog.product.update`). *(funde follow-up "permissão do upload do banner" da Fase 3)*

---

## Etapa 4 — Loja-demo por template + import de assets (backend)

> O **conteúdo demo** de cada template (categorias/produtos/imagens) vem de um manifesto **`demo.json`** no código do template (junto do `settings-schema.json`); as imagens **nascem com URL do uxpilot** e são **importadas pro CDN**. Doc [26](../concepts/26_template_system.md)/[27](../concepts/27_template_authoring_guide.md).

### Manifesto + import (doc [26](../concepts/26_template_system.md)/[13](../concepts/13_performance_cache_and_cdn.md))
...
### Loja-demo por template (doc [26](../concepts/26_template_system.md))
...
- [ ] **Aurora/Bazar/Studio** ganham sua loja-demo (carga artificial do `docs/design/`, pelo **mesmo caminho** de um template futuro — doc [27](../concepts/27_template_authoring_guide.md)).
...
## Etapa 5 — Vitrine lê os settings + preview navegável

### Vitrine (doc [10](../concepts/10_storefront_and_layouts.md)/[26](../concepts/26_template_system.md))
- [ ] Aurora/Bazar/Studio leem `theme.settings[key] ?? <default do template>` nos textos/imagens de chrome.
- [ ] **Campos de imagem com default no CDN** — a imagem original do template (importada na Etapa 4) preenche o default; nunca fundo vazio.

### Preview navegável (doc [26](../concepts/26_template_system.md))
- [ ] Botão **"ver preview completo"** no painel → abre, em **outra aba**, o **template navegável** (storefront servindo a **loja-demo** do template — Etapa 4) — cada clique funciona.
...
## Etapa 6 — Conteúdo das páginas (content_pages/menus/banners)
...
### Painel (doc [09](../concepts/09_merchant_dashboard.md))
...
### Vitrine (doc [10](../concepts/10_storefront_and_layouts.md))
...
- **Motor genérico de HTML** (cadastrar template 100% sem deploy) → fora da V1 (mantém o port React fiel — doc [26](../concepts/26_template_system.md)).
...
## Reconciliações
- A **personalização do lojista** (`P3-TPL-04`) é desta fase — **depende** do admin (Fase 4) registrar o template/schema/thumbnail. A Fase 3 entrega os 3 templates + picker + banner + preview por imagem. Decisões no doc [26](../concepts/26_template_system.md).
- O **conteúdo demo** (loja-demo por template + `import_assets`) e o **preview navegável** são desta fase — dependem da vitrine ler `theme.settings` + do catálogo demo (não dá pra ficar "bonito como o design" sem isso). Passo-a-passo de autoria no doc [27](../concepts/27_template_authoring_guide.md).
