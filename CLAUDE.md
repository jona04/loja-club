...
- **Excluir = soft delete, NUNCA via status:** remover/excluir é **sempre** `deleted_at` (soft delete, jamais hard delete) — fica no banco, some da visão do usuário. Um **status** (`archived`, `paused`, …) é **operacional/reversível** (recurso fora do ar) e **JAMAIS** significa exclusão. Ex.: produto `archived` = offline reversível; **deletar** produto/loja = `deleted_at`.
- **Status tem que ter serventia — nada de enfeite:** **todo** valor de status precisa ser **lido em código** (alguém decide algo com ele) **e** ter **referência documentada** do que faz. Status/flag que ninguém lê é lixo → remover.
